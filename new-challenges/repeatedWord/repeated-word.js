...
const repeatedWord = function (str) {
  ...
  // FIXME: Permission from Joh nto use .split() with regex.  Discussion to take place in classroom.

  let words = str.split(/\b/g);

  for (let i = 0; i < words.length; i++) {
    // console.log(words[i]);
    if (strMap.contains(words[i])) {
      return words[i];
    } else if (words[i] !== ' ') {
      strMap.add(words[i].toLowerCase(), words[i]);
      // console.log(strMap.map);
    }
  }
